...
class ElevatorControlSystem(object):
	...
	def pickup(self, floorNumber, direction):
		...
		""" 
		for selectedElevator in self.managedElevators:	
			# Priority Order: Check all running elevators in the same direction for pick up. Optimize efficiency.
			# Can be changed to call elevators no already in use instead.
			""" Find closest elevators traveling downwards """
			if direction == selectedElevator.direction and direction < 0 and selectedElevator.express == False:
				for selectedElevator in self.managedElevators:
					if selectedElevator == self.managedElevators[0]:
					 	closestElevator = selectedElevator
					elif selectedElevator.currentFloor - floorNumber < closestElevator.currentFloor - floorNumber:
					 	closestElevator = selectedElevator
			...
			elif direction == selectedElevator.direction and direction > 0 and selectedElevator.express == False:
				for selectedElevator in self.managedElevators:
					if selectedElevator == self.managedElevators[0]:
					 	closestElevator = selectedElevator
					elif floorNumber - selectedElevator.currentFloor < floorNumber - closestElevator.currentFloor:
					 	closestElevator = selectedElevator
